...
On Node, the SDK registers process handlers so buffered telemetry flushes on `beforeExit`, `SIGTERM`, `SIGINT`, and `uncaughtException`. You usually do not call `flush()` yourself. See the [SDK reference](../sdk/README.mdx) for the exit-path details.
